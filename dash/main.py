...
def state():
    return jsonify({'status': False, 'message': 'binary_sensor entities are read-only and cannot be toggled'}), 400
...
if __name__ == '__main__':
    ...
    try:
        socketio.run(app, host='0.0.0.0', port=4111, debug=False, allow_unsafe_werkzeug=True)
    finally:
